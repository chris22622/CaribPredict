...
import { getMarketImageUrl } from '@/lib/market-images';
...
export function Thumb({ market, size = 64, radius = 10, useAiPhoto = true }: { market: CpMarket; size?: number; radius?: number; useAiPhoto?: boolean }) {
  ...
  // Prefer AI-generated Pollinations photo (matches what's on production).
  if (useAiPhoto && market.id && market.categoryDb && market.countryDb && market.question) {
    const url = getMarketImageUrl(market.id, market.categoryDb, market.countryDb, market.question, Math.max(size * 2, 256), Math.max(size * 2, 256));
    return (
