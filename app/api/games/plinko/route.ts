// One-shot Plinko bet. Settles instantly. Returns the full path so the UI
// can animate the ball drop, plus seed + hash for verification.

import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@supabase/supabase-js';
import {
  dropPlinko, makeServerSeed, instantPayoutCents, sha256Hex,
  plinkoMultipliers, PlinkoRisk, PLINKO_ROWS,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { userId, rows, risk, amountUsdt, clientSeed } = body as {
      userId?: string; rows?: number; risk?: PlinkoRisk;
      amountUsdt?: number; clientSeed?: string;
    };
    if (!userId) return NextResponse.json({ error: 'userId required' }, { status: 400 });
    if (!rows || !(PLINKO_ROWS as readonly number[]).includes(rows)) {
      return NextResponse.json({ error: `rows must be one of ${PLINKO_ROWS.join(', ')}` }, { status: 400 });
    }
    if (risk !== 'low' && risk !== 'medium' && risk !== 'high') {
      return NextResponse.json({ error: 'risk must be low | medium | high' }, { status: 400 });
    }
    ...
    const result = dropPlinko(
      { server_seed: seed, server_seed_hash: hash, client_seed: cs, nonce },
      rows, risk,
    );
    ...
    await supabase.from('instant_game_bets').insert({
      ...
      nonce,
      params: { rows, risk },
      result: { path: result.path, bin: result.bin },
